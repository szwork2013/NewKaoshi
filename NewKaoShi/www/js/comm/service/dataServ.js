...
GetPaperData:GetPaperData
		...
		function SavePaper(data) {
			SqliteServ.transaction(function(tx) {
				var len = data.length;
				for (var i = 0; i < len; i++) {
					SqliteServ.saveOrupadte(tx, 'tb_Papers', ["PaperID","PaperName","ExamTypeID", "PaperTypeID", "TotalScore", "ItemNum", "UserCount", "IsUpdate","IsVip","IsDownLoad", "PassMark", "UpLoaderID", "TotalTime", "Year","ContainQuestionTypes","CreateTime","CreatorID","UpdateTime","UpdaterID"], [data[i].PaperID,data[i].PaperName,data[i].ExamTypeID,data[i].PaperTypeID,data[i].TotalScore,data[i].ItemNum,data[i].UserCount,data[i].IsUpdate,data[i].IsVip,data[i].IsDownLoad,data[i].PassMark,data[i].UpLoaderID,data[i].TotalTime,data[i].Year,data[i].ContainQuestionTypes,data[i].CreateTime,data[i].CreatorID,data[i].UpdateTime,data[i].UpdaterID], "PaperID=?", [data[i].PaperID]);
				}
			})
		}
		//存储试题数据
